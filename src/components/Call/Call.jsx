...
function Call() {
  ...
  // ✅ STEP 1: Define endChat FIRST (before handleInsufficientBalance uses it)
  const endChat = useCallback(async () => {
    try {
      // Send message to other participant
      await agora.sendMessage?.(`${SYSTEM_MESSAGES.CHAT_ENDED}Chat session has ended`);
      
      // Notify backend that chat ended (clear busy state)
      await notifyBackendCallEnded();
      ...
      await finalizeChatBilling(channelName, agora.callDuration, callType, 0);
      agora.cleanup();
  ...
  }, [agora, notifyBackendCallEnded, channelName, callType, ui, setCallConnected, setCallEnded, setCallStatus, setShowEndCallPopup]);

  // ✅ STEP 2: Define endCall SECOND (before handleInsufficientBalance uses it)
  const endCall = useCallback(async () => {
    try {
      // Stop recording if active
      controls.stopRecording?.();
      ...
      // Notify backend that call ended (clear busy state)
      await notifyBackendCallEnded();
      ...
      await finalizeCallBilling(channelName, agora.callDuration, callType, 0);
      agora.cleanup();
  ...
  }, [controls, agora, notifyBackendCallEnded, channelName, callType, setCallConnected, setCallEnded, setCallStatus, setShowEndCallPopup]);

  // ✅ STEP 3: Define handleInsufficientBalance THIRD (after endChat and endCall are defined)
  ...
  // ✅ STEP 4: NOW billing hook can use handleInsufficientBalance safely
  const billing = useCallBilling(
    channelName,
    callType,
    agora.callDuration,
