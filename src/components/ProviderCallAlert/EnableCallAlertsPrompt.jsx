...
const EnableCallAlertsPrompt = () => {
  ...
  const { unlock } = useSimpleAudio();

  useEffect(() => {
    const enabled = sessionStorage.getItem('audioUnlocked') === 'true';
    if (!enabled) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: '14px' }}>
        🔔 <strong>Enable Call Alerts</strong> - Tap to hear ringtone for incoming calls
      </span>
      <button
