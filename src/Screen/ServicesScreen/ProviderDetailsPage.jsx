...
const SkeletonCard = memo(({ className = "" }) => (
  <div className={`bg-white rounded-xl border border-gray-200 p-4 sm:p-6 ${className}`}>
    <div className="space-y-3 sm:space-y-4">
      <div className="h-3 sm:h-4 bg-gray-200 rounded w-1/3 animate-pulse" />
      <div className="h-2.5 sm:h-3 bg-gray-200 rounded w-2/3 animate-pulse" style={{ animationDelay: '150ms' }} />
      <div className="h-2.5 sm:h-3 bg-gray-200 rounded w-1/2 animate-pulse" style={{ animationDelay: '300ms' }} />
    </div>
  </div>
));
  ...
  <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 md:p-8">
    <div className="flex flex-col md:flex-row gap-4 sm:gap-6">
      ...
      <div className="flex-1 space-y-2 sm:space-y-3">
        <div className="h-5 sm:h-6 bg-gray-200 rounded w-1/2 animate-pulse" />
        <div className="h-3 sm:h-4 bg-gray-200 rounded w-1/3 animate-pulse" style={{ animationDelay: '150ms' }} />
        <div className="flex gap-2 sm:gap-3">
          <div className="h-7 sm:h-8 bg-gray-200 rounded w-20 sm:w-24 animate-pulse" style={{ animationDelay: '300ms' }} />
          <div className="h-7 sm:h-8 bg-gray-200 rounded w-20 sm:w-24 animate-pulse" style={{ animationDelay: '450ms' }} />
        </div>
      </div>
    </div>
  </div>
...
const LoginPrompt = memo(({ mode, onLoginClick }) => (
  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200 p-4 sm:p-6">
    <div className="text-center">
      ...
      <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2">Login Required</h3>
      <p className="text-xs sm:text-sm text-gray-600 mb-3 sm:mb-4">
        Please login to book a {MODE_LABELS[mode] || 'session'} with this provider
      </p>
      <button
        onClick={onLoginClick}
        className="w-full py-2.5 sm:py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm sm:text-base font-semibold transition-colors flex items-center justify-center gap-2"
      >
...
const StarRating = memo(({ rating = 0, size = "w-3 h-3 sm:w-3.5 sm:h-3.5" }) => (
  <>
    {Array.from({ length: 5 }, (_, i) => (
      <Star
        key={i}
...
const ProfileImage = memo(({ provider }) => (
  <div className="flex-shrink-0">
    <div className="relative">
      <div className="flex items-center justify-center text-white text-xl sm:text-2xl font-bold">
        {provider.personalInfo.profileImage ? (
          <img
            src={provider.personalInfo.profileImage}
            alt="Profile"
...
const StatCard = memo(({ icon: Icon, value, label, iconColor }) => (
  <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-white-200 p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
    ...
    <div className="min-w-0">
      ...
      <div className="text-xs text-white mt-0.5 sm:mt-1 truncate">{label}</div>
    </div>
  </div>
));

const InfoBadge = memo(({ icon: Icon, text, iconColor = "text-blue-600" }) => (
  <div className="flex items-center gap-1 sm:gap-1.5 bg-gray-50 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg">
    <Icon className={`w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0 ${iconColor}`} />
    <span className="text-xs sm:text-sm text-gray-900 truncate">{text}</span>
  </div>
));
...
const BookingButton = memo(({
  ...
  const getButtonContent = () => {
    ...
    if (walletCheckLoading) {
      return (
        <>
          <Loader2 className="w-4 h-4 animate-spin" />
          <span className="hidden sm:inline">Sign For Booking</span>
        </>
      );
    }
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`w-full py-2.5 sm:py-3.5 rounded-xl text-xs sm:text-sm font-semibold text-white transition-all duration-200 shadow-md flex items-center justify-center gap-2 
        ${disabled
          ? 'bg-gray-400 cursor-not-allowed opacity-60'
          : `${modeConfig.buttonClass} hover:shadow-lg transform hover:-translate-y-0.5`
        }`}
...
const ProviderHeader = memo(({ provider }) => (
  <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-gray-200 overflow-hidden">
    ...
    <div className="relative w-full h-40 sm:h-48 md:h-60 lg:h-72">
      ...
      {/* Floating Profile Image */}
      <div className="absolute left-4 sm:left-6 bottom-[-30px] sm:bottom-[-35px]">
        <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-full border-3 sm:border-4 border-white shadow-lg overflow-hidden">
          <ProfileImage provider={provider} />
        </div>
      </div>
    </div>
    ...
    {/* ================== BODY SECTION ================== */}
    <div className="pt-12 sm:pt-14 px-4 sm:px-6 pb-4 sm:pb-6 md:px-10">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 lg:gap-6">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-white text-xs sm:text-sm font-medium mt-0.5 truncate">
            {provider.services?.primary || "Service Provider"}
          </p>

          <p className="text-white text-xs sm:text-sm font-medium mt-1 truncate">
            {provider.services?.category || "General Services"}
          </p>
          ...
          <div className="mt-4 space-y-2 sm:space-y-3">
            {sequence.map((key) => {
              ...
              return (
                <div
                  key={key}
                  className="grid grid-cols-[90px_15px_1fr] sm:grid-cols-[120px_20px_1fr] md:grid-cols-[150px_20px_1fr] text-xs sm:text-sm items-start gap-1"
                >
                  <span className="font-semibold text-white truncate">{label}</span>
                  <span className="text-white text-center">:</span>
                  <span className="text-white break-words">{formattedValue}</span>
                </div>
              );
            })}
          </div>
        ...
        {/* ===== RIGHT SIDE: VERIFIED + LOCATION + RATING ===== */}
        <div className="flex flex-row lg:flex-col items-start lg:items-end justify-between lg:justify-start gap-3 lg:min-w-[200px]">

          {/* Verified Badge */}
          {provider.professional?.verified && (
            <span className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 bg-green-50 text-green-700 border border-green-200 rounded-lg text-xs font-medium whitespace-nowrap">
          ...
          {/* Rating Section */}
          <div className="flex flex-col items-start lg:items-end gap-1.5 sm:gap-2">
            {/* Rating Badge */}
            <div className="flex items-center gap-1 sm:gap-1.5 bg-amber-50 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg">
              <span className="ml-0.5 sm:ml-1 font-bold text-white text-xs sm:text-sm">
                {provider.ratings?.overall || 0}
              </span>
              <StarRating rating={provider.ratings?.overall || 0} size="w-3 h-3 sm:w-3.5 sm:h-3.5" />
            </div>
...
const AboutSection = memo(({ provider }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-gray-200 p-4 sm:p-6">
...
const VerificationSection = memo(() => {
  ...
  return (
    <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-green-600 p-4 sm:p-6">
      ...
      <div className="space-y-2 sm:space-y-3">
        {sequence.map((key) => {
          ...
          return (
            <div
              key={key}
              className="grid grid-cols-[1fr_auto_auto] sm:grid-cols-3 items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 bg-white rounded-lg border border-gray-300"
            >
              {/* 1. Label */}
              <span className="text-black text-xs sm:text-sm font-semibold truncate">
                {labelFormat[key]}
              </span>
              ...
              <span
                className={`text-xs sm:text-sm font-semibold ${
                  verified ? "text-green-700" : "text-white"
                }`}
              >
                {status}
              </span>
              ...
              <div className="flex justify-end">
                {verified ? (
                  ...
                  <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-white opacity-50" />
                )}
              </div>
            </div>
          );
        ...
        {testimonials.map((review, idx) => (
          <div key={idx} className="p-3 sm:p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div className="flex items-start gap-2 sm:gap-3 mb-2">
              <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-xs sm:text-sm font-semibold flex-shrink-0">
                {review.client?.charAt(0) || 'U'}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-1">
                  ...
                  {review.verified && (
                    <CheckCircle className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-green-500 flex-shrink-0" />
                  )}
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <div className="flex items-center gap-0.5">
                    <StarRating rating={review.rating || 0} size="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                  </div>
                  <span className="text-xs text-gray-500">{review.date || 'N/A'}</span>
                </div>
              </div>
...
const AchievementsSection = memo(({ achievements }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-gray-200 p-4 sm:p-6">
      ...
      <div className="space-y-2">
        {achievements.map((achievement, idx) => (
          <div key={idx} className="flex items-start gap-2 p-2 sm:p-2.5 bg-white rounded-lg">
            <Award className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-yellow-600 flex-shrink-0 mt-0.5" />
            <span className="text-xs text-gray-900 leading-snug">{achievement}</span>
          </div>
        ))}
      </div>
    </div>
...
const BookingCard = memo(({
  ...
  return (
    <div className="rounded-xl shadow-lg overflow-hidden">
      <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white backdrop-blur-sm p-4 sm:p-6">
        {/* Mode Header */}
        <div className="flex items-center gap-2 mb-3 sm:mb-4">
          <div className="p-1.5 sm:p-2 rounded-lg">
            <ModeIcon className={`w-4 h-4 sm:w-5 sm:h-5 ${modeConfig.textColor}`} />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-sm sm:text-base font-bold text-white truncate">{modeConfig.label}</h3>
            <p className="text-xs text-white">Book your session</p>
          </div>
        </div>

        {/* Price Display */}
        <div className="p-3 sm:p-4 bg-white rounded-xl border mb-3 sm:mb-4">
          <div className="flex items-baseline justify-between gap-2">
            <div className="min-w-0 flex-1">
              <div className="text-xs text-gray-600 mb-1">Session Price</div>
              <div className={`text-2xl sm:text-3xl font-bold ${modeConfig.textColor} truncate`}>
                ₹{currentPrice}
                {(mode === 'call' || mode === 'video' || mode === 'chat') && (
                  <span className="text-xs sm:text-sm font-medium text-gray-500 ml-1">/min</span>
                )}
              </div>
            </div>
            <div
              className={`px-2 sm:px-3 py-1 bg-white text-gray-900 ${modeConfig.borderColor} border rounded-full text-xs font-semibold uppercase whitespace-nowrap`}
            >
              {mode}
            </div>
          </div>
    ...
    <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-gray-200 p-4 sm:p-6">
      ...
      <div className="flex flex-wrap gap-2">
        {languages.map((lang, idx) => (
          <span
            key={idx}
            className="px-2 sm:px-2.5 py-1 bg-gray-[#F0F0F0] text-cyan-700 border border-cyan-200 rounded-lg text-xs font-medium"
          >
            {lang}
          </span>
        ))}
  ...
  return (
    <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-gray-200 p-4 sm:p-6">
      ...
      <div className="space-y-2">
        {equipment.map((item, idx) => (
          <div key={idx} className="flex items-center gap-2 text-xs text-gray-900 p-2 bg-white rounded-lg">
            <CheckCircle className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-orange-600 flex-shrink-0" />
    ...
    <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-gray-200 p-4 sm:p-6">
      ...
      <div className="flex flex-wrap gap-2">
        {serviceAreas.map((area, idx) => (
          <span
            key={idx}
            className="text-xs px-2 sm:px-2.5 py-1 bg-white text-gray-900 rounded-lg"
          >
            {area}
          </span>
        ))}
...
const TrustBadgeCard = memo(({ provider, memberSince }) => (
  <div className="bg-gradient-to-br from-[#0B1A47] via-[#672255] to-[#B34C44] text-white rounded-xl border border-gray-200 p-4 sm:p-6">
    <h3 className="text-sm sm:text-base font-bold text-white mb-2 sm:mb-3 flex items-center gap-2">
      <Shield className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-600" />
      Trust & Safety
    </h3>
    {provider.professional?.verificationDocuments?.length > 0 ? (
      <div className="space-y-2">
        {provider.professional.verificationDocuments.map((doc, idx) => (
          <div key={idx} className="flex items-center gap-2 text-xs text-gray-900 p-2 bg-white rounded-lg">
            <CheckCircle className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-purple-600 flex-shrink-0" />
            <span>{doc} Verified</span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-2xl z-50 safe-area-bottom">
      <div className="px-3 sm:px-4 py-2.5 sm:py-3">
        {/* Mode Indicator */}
        <div className={`flex items-center gap-1.5 sm:gap-2 mb-2 px-2.5 sm:px-3 py-1.5 ${modeConfig.bgColor} ${modeConfig.borderColor} border rounded-lg`}>
          <ModeIcon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${modeConfig.textColor}`} />
          <span className={`text-xs font-semibold ${modeConfig.textColor} truncate flex-1`}>
            {modeConfig.label}
          </span>
          <div className="flex items-center gap-1 sm:gap-1.5">
            <div className={`w-1.5 h-1.5 ${modeConfig.dotColor} rounded-full animate-pulse`} />
            <span className={`text-xs ${modeConfig.textColor} whitespace-nowrap`}>
              {provider.availability?.status === 'available' ? 'Available' : 'Unavailable'}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex-shrink-0">
            <div className="text-xs text-gray-600">From</div>
            <div className={`text-lg sm:text-xl font-bold ${modeConfig.textColor}`}>
              ₹{currentPrice}
              {(mode === 'call' || mode === 'video' || mode === 'chat') && (
                <span className="text-xs sm:text-sm font-medium text-gray-500 ml-0.5 sm:ml-1">/min</span>
              )}
            </div>
          </div>
          <button
            onClick={onBookingAction}
            disabled={isBookingDisabled}
            className={`flex-1 py-2.5 sm:py-3.5 rounded-xl text-xs sm:text-sm font-semibold text-white transition-all duration-200 shadow-md flex items-center justify-center gap-1.5 sm:gap-2 
              ${isBookingDisabled
                ? 'bg-gray-400 cursor-not-allowed opacity-60'
                : `${modeConfig.buttonClass} active:scale-95`
              }`}
...
const ProviderDetailsPage = () => {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 pb-20 sm:pb-24 lg:pb-0">
        <main className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 md:py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            {/* Main Content */}
            <div className="lg:col-span-2 space-y-4 sm:space-y-6">
