...
const TransactionsTab = (provider) => {
  ...
  const formatAmount = (transaction) => {
    ...
    // Swap values: if type is credit, show it as debit display (- and red)
    // if type is debit, show it as credit display (+ and green)
  ...
  const getTypeBadge = (type) => {
    ...
    // Swap colors: credit shows as red (debit color), debit shows as green (credit color)
    return (
      <span
      ...
      {transactions.length > 0 && (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl shadow-sm border border-blue-200 p-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center">
              <div className="text-sm text-gray-600 font-medium">Total Earning</div>
                ...
                <tr>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1">
                      {totals.totalDebit > 0 && (
                        <span className="text-green-600">+₹{totals.totalDebit.toLocaleString('en-IN')}</span>
                      )}
                      {totals.totalCredit > 0 && (
                        <span className="text-red-600">-₹{totals.totalCredit.toLocaleString('en-IN')}</span>
                      )}
                    </div>
                  </td>
